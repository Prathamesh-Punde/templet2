...
// Particle Canvas Animation - Disabled for professional theme
// const canvas = document.getElementById('particleCanvas');
// Animation removed as per professional design requirements

// Product Showcase Card Rotation
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            const timer = setInterval(() => {
                count += increment;
                if (count >= target) {
                    entry.target.textContent = target + (target === 9 ? '' : '+');
                    clearInterval(timer);
                } else {
                    entry.target.textContent = Math.floor(count) + (target === 9 ? '' : '+');
                }
            }, 30);
...
if (contactForm) {
    ...
    if (serviceParam && serviceSelect) {
        // Set the dropdown value based on URL parameter
        serviceSelect.value = serviceParam;
        
        // Highlight the selected option with a smooth scroll
        serviceSelect.focus();
        setTimeout(() => serviceSelect.blur(), 100);
    }
